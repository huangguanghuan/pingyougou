...
app.controller('goodsController', function($scope, $controller, baseService){
    ...
    $scope.findItemCatByParentId=function (parentId,name) {
        baseService.sendGet("/itemCat/findItemCatByParentId","parentId=" + parentId).then(
            function (response) {
                // 获取响应数据
  ...
  $scope.$watch('goods.category1Id',function (newValue,oldValue) {
    ...
    //** 监控模板id $scope.goods.typeTemplateId  变量，查询对应的品牌*/
    $scope.$watch('goods.typeTemplateId',function (newValue,oldValue) {
        if(newValue){baseService.sendGet("/typeTemplate/findOne?id="+newValue).then(function (response) {
                //获取模板中的品牌列表
